...
\FORALL{face in faces}
    \STATE $g_data\gets$ guard set of the face
    \IF{$g_data$ is empty}
        \STATE Continue
    \ENDIF
    ...
    \FORALL{half\_edge in face}
        \STATE $twin_g_data\gets$ guard set of the twin face of the half\_edge
        \IF{$twin_g_data$ is empty}
            \STATE Continue
        \ENDIF
        \IF{face guard set includes $twin_g_data$}
            \STATE $is\_shadow\gets \FALSE$
        \ENDIF
        \IF{twin guard set data includes face guard set}
            \STATE $is\_light\gets \FALSE$
        \ENDIF
    \ENDFOR
    \STATE $all\_witness\_to\_guards[all\_index]\gets$ face guard set
