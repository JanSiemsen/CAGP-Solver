...
\STATE $firstHalf = guards[:half]$ 
\STATE $secondHalf = guards[half:]$
\IF{$len(guards) = 1$}
    \STATE $Arrangement(guards[0][0], guards[0][1])$
\ELSE
    \STATE $generate\_AVP\_recursive(firstHalf).overlaygenerate\_AVP\_recursive(secondHalf)$
\En
\end{algorithmic}
\end{algorithm}
