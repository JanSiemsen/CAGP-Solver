...
\begin{algorithmic}[1]
\Procedure{AVP\_processing}{$faces$ of the AVP arrangement}
...
\begin{algorithmic}[1]
\Procedure{edge\_clique\_covers}{$G, K$}
...
\Procedure{solve}{$color\_lim$}
    ...
    \While{$missing\_witnesses$}
        ...
        \State $missing\_witnesses \gets \_\_check\_coverage(solution)$
    \EndWhile
    \State $solution \gets [seen.setdefault(g[0], g) \text{ for } g \text{ in } solution \text{ if } g[0] \text{ not in } seen]$
...
\Procedure{binary\_search}{}
    ...
    \While{$lower < upper$}
        ...
        \State $solution \gets \_\_solve(assumptions=\_\_deactivate\_guards(mid))$
        \If{$solution$}
    ...
    \If{not $optimal\_solution$}
        \State $optimal\_solution \gets \_\_solve(assumptions=\_\_deactivate\_guards(upper))$
    \EndIf
    \State \Return $lower, optimal\_solution$
\EndProcedure
\Procedure{linear\_ascent}{$color\_lim$}
    \State $solution \gets \_\_solve(assumptions=\_\_deactivate\_guards(color\_lim))$
