...
\begin{algorithmic}[1]
\REQUIRE $guard\_to\_witnesses, initial\_witnesses, G_{vis}$
\STATE Initialize empty set: $solution$
\STATE $uncovered\gets initial\_witnesses$
\STATE $size\gets 0$
\WHILE{$uncovered$ is not empty}
    \STATE Initialize weights with pair (g\_id, size of intersection of witnesses and uncovered) for each g\_id, witnesses in guard\_to\_witnesses
    \STATE Initialize solver with weights and G
    \STATE Solve the solver and get independent\_set
    \FORALL{guard in independent\_set}
        \STATE Add pair (guard, size) to solution
        \STATE Remove guard\_to\_witnesses[guard] from uncovered
    \ENDFOR
