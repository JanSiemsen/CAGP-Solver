...
\STATE Initialize empty list: faces
\FORALL{face in self}
    \STATE Add face to faces
\ENDFOR
\STATE Initialize index to size of guard
\STATE Initialize all\_index to size of guard
\FORALL{face in faces}
    \IF{face data is empty}
        \STATE Continue to next iteration
    \ENDIF
    \STATE Initialize is\_shadow and is\_light to true
    \FORALL{half\_edge in face}
        \IF{twin face data is empty}
            \STATE Continue to next iteration
        \ENDIF
        \IF{face data includes twin face data}
            \STATE Set is\_shadow to false
        \ENDIF
        \IF{twin face data includes face data}
            \STATE Set is\_light to false
        \ENDIF
    \ENDFOR
    \STATE Insert all\_index and face data to all\_witness\_to\_guards
    \FORALL{g in face data}
        \STATE Insert all\_index to all\_guard\_to\_witnesses[g]
    \ENDFOR
    \STATE Increment all\_index
    \IF{is\_shadow is true}
        \STATE Insert index and face data to witness\_to\_guards
        \FORALL{g in face data}
            \STATE Insert index to guard\_to\_witnesses[g]
        \ENDFOR
        \STATE Increment index
    \ENDIF
    \IF{is\_light is true}
        \STATE Add face data to light\_guard\_sets
    \ENDIF
\ENDFOR
\STATE Create result tuple with witness\_to\_guards, guard\_to\_witnesses, light\_guard\_sets, all\_witness\_to\_guards, all\_guard\_to\_witnesses
\RETURN result
\end{algorithmic}
\end{algorithm}
