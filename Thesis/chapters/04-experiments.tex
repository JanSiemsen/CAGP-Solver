\chapter{Experiments}
\section{Experiment Environment}
All geometric operations were implemented using the C++ library CGAL. The visibility graph creation as well as all solvers were implemented in Python (version 3.10.14). For the MIP solvers, we used Gurobi (gurobipy version 11.0.1). The SAT solvers were implemented using PySAT (python-sat version 0.1.8.dev9). Lastly, the CP-SAT solvers were implemented with OR-tools CP-SAT solver (ortools version 9.9.3963).
The benchmarks were run in WSL2 (Windows 11, version 22H2) using an AMD Ryzen 7 7800X3D 8-Core Processor 4.20 GHz and the WSL was assigned 28 GB of RAM. All solvers were given a maximum time of 600 seconds.

\section{SAT Solver Choice}
PySAT provides several different SAT solvers. That is why in this section, we will compare them against each other to determine, which one performs the best on our model. Additionally, we will compare using the clauses \cref{eq_SAT:f.3}(CAGP)/\cref{eq_SAT_cf:f.3}(CFCAGP) (version 2) against leaving them out and filtering the solution afterward as described in the SAT Color Optimization chapter (version 1). Note that for the card solvers (Gluecard3, Gluecard3, Minicard), we used the natively provided add\_atmost(variables, 1) clauses to model \cref{eq_SAT:f.3}(CAGP)/\cref{eq_SAT_cf:f.3}(CFCAGP). Due to the parameter sets becoming quite large when comparing all of the SAT solvers, we filtered out a few beforehand that performed poorly during testing. The testing was done using random simple polygons from 100 to 500 vertices and from 10 to 50 holes and each instance size had 30 polygons ~\cite{art-gallery-unicamp-page}. When plotting the solvers in a cactus plot, we can see that both version 1 and version 2 with CaDiCaL103 perform well compared to the other solvers \cref{fig:cactus_SAT}. Indeed, they both were able to solve all instances \cref{tab:SAT_num_solved_v1}/\cref{tab:SAT_num_solved_v2}. But looking at the mean runtimes one cannot determine a clear best solver \cref{tab:mean_low_high_SAT_v1}/\cref{tab:mean_low_high_SAT_v2}. One can also notice that even between instances of the same size there can be a big variance between lowest and highest runtime. As most SAT solvers run on a single core, we can run multiple of them at the same time on a multi-core system and stop as soon as one of them finds a solution. First up, we choose CaDiCaL103 version 1 and version 2. Additionally, by looking at the 3 best alternatives for the 20 worst instances for CaDiCaL103, we choose Glucose42 version 1 and version 2 as well as Glucose4 version 2. These five solvers will be used in parallel in the comparison between MIP vs. SAT vs. CP-SAT.

\begin{figure}[htbp]
\centering
\includegraphics[scale=0.7]{Thesis/figures/minibenchmark_cactus_plot_runtime_SAT_with_holes.png}
\caption{Cactus Plot Comparing the SAT Solver Performance}
\label{fig:cactus_SAT}
\end{figure}

\begin{table}[htbp]
\fontsize{4}{4}\selectfont
\centering
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{3} % Increase row spacing
\begin{tabular}{lrrrrrrrrr}
\toprule
\makecell{vertices} & \makecell{Cadical103} & \makecell{Cadical153} & \makecell{Glucose4} & \makecell{Glucose42} & \makecell{Gluecard4} & \makecell{MapleChrono} & \makecell{MergeSat3} & \makecell{Minicard} & \makecell{Minisat22} \\
% vertices &  &  &  &  &  &  &  &  &  \\
\midrule
\makecell{100} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} \\
\makecell{200} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{27} & \makecell{30} & \makecell{30} & \makecell{30} \\
\makecell{300} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{19} & \makecell{30} & \makecell{28} & \makecell{28} \\
\makecell{400} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} \\
\makecell{500} & \makecell{30} & \makecell{29} & \makecell{29} & \makecell{29} & \makecell{29} & \makecell{19} & \makecell{28} & \makecell{26} & \makecell{26} \\
\bottomrule
\end{tabular}
\caption{Number of instances solved by the version 1 solvers}
\label{tab:SAT_num_solved_v1}
\end{table}

\begin{table}[htbp]
\fontsize{4}{4}\selectfont
\centering
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{3} % Increase row spacing
\begin{tabular}{lrrrrrrrrr}
\toprule
\makecell{vertices} & \makecell{Cadical103} & \makecell{Cadical153} & \makecell{Glucose4} & \makecell{Glucose42} & \makecell{Gluecard4} & \makecell{MapleChrono} & \makecell{MergeSat3} & \makecell{Minicard} & \makecell{Minisat22} \\
% vertices &  &  &  &  &  &  &  &  &  \\
\midrule
\makecell{100} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} \\
\makecell{200} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{27} & \makecell{30} & \makecell{30} & \makecell{30} \\
\makecell{300} & \makecell{30} & \makecell{30} & \makecell{29} & \makecell{30} & \makecell{29} & \makecell{20} & \makecell{30} & \makecell{27} & \makecell{27} \\
\makecell{400} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{29} & \makecell{30} \\
\makecell{500} & \makecell{30} & \makecell{30} & \makecell{28} & \makecell{29} & \makecell{28} & \makecell{21} & \makecell{28} & \makecell{27} & \makecell{28} \\
\bottomrule
\end{tabular}
\caption{Number of instances solved by the version 2 solvers}
\label{tab:SAT_num_solved_v2}
\end{table}


\begin{table}[htbp]
\fontsize{4}{4}\selectfont
\centering
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{3} % Increase row spacing
\begin{tabular}{lrrrrrrr}
\toprule
\makecell{vertices} & \makecell{Cadical103} & \makecell{Cadical153} & \makecell{Glucose4} & \makecell{Glucose42} & \makecell{MergeSat3} & \makecell{Minicard} & \makecell{Minisat22} \\
% vertices &  &  &  &  &  &  &  \\
\midrule
\makecell{100} & \makecell{4} & \makecell{0} & \makecell{1} & \makecell{3} & \makecell{0} & \makecell{2} & \makecell{1} \\
\makecell{200} & \makecell{2} & \makecell{2} & \makecell{4} & \makecell{6} & \makecell{0} & \makecell{1} & \makecell{1} \\
\makecell{300} & \makecell{7} & \makecell{3} & \makecell{0} & \makecell{2} & \makecell{1} & \makecell{1} & \makecell{0} \\
\makecell{400} & \makecell{5} & \makecell{1} & \makecell{3} & \makecell{6} & \makecell{0} & \makecell{0} & \makecell{0} \\
\makecell{500} & \makecell{4} & \makecell{0} & \makecell{1} & \makecell{3} & \makecell{0} & \makecell{2} & \makecell{0} \\
\bottomrule
\end{tabular}
\caption{Number of instances with the lowest runtime for version 1 solvers}
\label{tab:SAT_small_time_v1}
\end{table}


\begin{table}[htbp]
\fontsize{4}{4}\selectfont
\centering
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{3} % Increase row spacing
\begin{tabular}{lrrrrrrrrr}
\toprule
\makecell{vertices} & \makecell{Cadical103} & \makecell{Cadical153} & \makecell{Glucose4} & \makecell{Glucose42} & \makecell{Gluecard4} & \makecell{MapleChrono} & \makecell{MergeSat3} & \makecell{Minicard} & \makecell{Minisat22} \\
% vertices &  &  &  &  &  &  &  &  &  \\
\midrule
\makecell{100} & \makecell{3} & \makecell{2} & \makecell{0} & \makecell{4} & \makecell{2} & \makecell{0} & \makecell{0} & \makecell{3} & \makecell{5} \\
\makecell{200} & \makecell{6} & \makecell{1} & \makecell{1} & \makecell{2} & \makecell{2} & \makecell{0} & \makecell{1} & \makecell{0} & \makecell{1} \\
\makecell{300} & \makecell{9} & \makecell{2} & \makecell{1} & \makecell{2} & \makecell{1} & \makecell{0} & \makecell{0} & \makecell{0} & \makecell{1} \\
\makecell{400} & \makecell{3} & \makecell{3} & \makecell{1} & \makecell{3} & \makecell{4} & \makecell{0} & \makecell{0} & \makecell{1} & \makecell{0} \\
\makecell{500} & \makecell{10} & \makecell{0} & \makecell{1} & \makecell{4} & \makecell{1} & \makecell{2} & \makecell{1} & \makecell{0} & \makecell{1} \\
\bottomrule
\end{tabular}
\caption{Number of instances with the lowest runtime for version 2 solvers}
\label{tab:SAT_small_time_v2}
\end{table}



\begin{table}[htbp]
\fontsize{4}{4}\selectfont
\centering
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{3} % Increase row spacing
\begin{tabular}{llllllllll}
\toprule
\makecell{vertices} & \makecell{Cadical103} & \makecell{Cadical153} & \makecell{Glucose4} & \makecell{Glucose42} & \makecell{Gluecard4} & \makecell{MapleChrono} & \makecell{MergeSat3} & \makecell{Minicard} & \makecell{Minisat22} \\
% vertices &  &  &  &  &  &  &  &  &  \\
\midrule
\makecell{100} & \makecell{0.31\\(0.02, 1.21)} & \makecell{0.40\\(0.02, 2.05)} & \makecell{0.51\\(0.01, 2.69)} & \makecell{0.43\\(0.01, 2.33)} & \makecell{0.53\\(0.01, 2.76)} & \makecell{3.92\\(1.74, 10.06)} & \makecell{2.37\\(0.03, 5.63)} & \makecell{0.50\\(0.01, 2.50)} & \makecell{0.50\\(0.01, 2.49)} \\
\makecell{200} & \makecell{8.38\\(0.25, 126.02)} & \makecell{13.56\\(0.20, 193.29)} & \makecell{7.15\\(0.10, 93.77)} & \makecell{6.63\\(0.18, 83.31)} & \makecell{7.24\\(0.10, 94.73)} & \makecell{5.76\\(2.15, 32.44)} & \makecell{11.21\\(1.80, 140.99)} & \makecell{22.95\\(0.10, 299.35)} & \makecell{22.84\\(0.10, 297.74)} \\
\makecell{300} & \makecell{27.56\\(0.36, 143.56)} & \makecell{46.65\\(0.32, 259.42)} & \makecell{66.18\\(0.22, 545.91)} & \makecell{56.65\\(0.27, 414.39)} & \makecell{66.65\\(0.22, 552.79)} & \makecell{32.55\\(3.14, 208.97)} & \makecell{49.19\\(1.63, 362.41)} & \makecell{84.55\\(0.18, 569.67)} & \makecell{83.34\\(0.18, 559.12)} \\
\makecell{400} & \makecell{6.42\\(0.52, 49.18)} & \makecell{9.18\\(0.50, 77.48)} & \makecell{7.59\\(0.14, 53.58)} & \makecell{5.94\\(0.26, 51.72)} & \makecell{7.76\\(0.15, 55.90)} & \makecell{11.11\\(2.78, 57.11)} & \makecell{9.26\\(1.71, 59.87)} & \makecell{36.02\\(0.27, 273.44)} & \makecell{36.22\\(0.27, 271.87)} \\
\makecell{500} & \makecell{40.92\\(0.39, 534.00)} & \makecell{54.20\\(0.51, 347.89)} & \makecell{55.87\\(0.21, 496.94)} & \makecell{46.15\\(0.43, 295.56)} & \makecell{56.47\\(0.22, 501.75)} & \makecell{75.73\\(4.30, 564.11)} & \makecell{56.32\\(1.62, 470.89)} & \makecell{129.31\\(0.17, 494.79)} & \makecell{128.88\\(0.17, 497.51)} \\
\bottomrule
\end{tabular}
\caption{Mean (lowest, highest) runtime for version 1 solvers (SAT)}
\label{tab:mean_low_high_SAT_v1}
\end{table}

\begin{table}[htbp]
\fontsize{4}{4}\selectfont
\centering
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{3} % Increase row spacing
\begin{tabular}{llllllllll}
\toprule
\makecell{vertices} & \makecell{Cadical103} & \makecell{Cadical153} & \makecell{Glucose4} & \makecell{Glucose42} & \makecell{Gluecard4} & \makecell{MapleChrono} & \makecell{MergeSat3} & \makecell{Minicard} & \makecell{Minisat22} \\
% vertices &  &  &  &  &  &  &  &  &  \\
\midrule
\makecell{100} & \makecell{0.39\\(0.02, 1.69)} & \makecell{0.41\\(0.01, 1.73)} & \makecell{0.52\\(0.01, 2.72)} & \makecell{0.45\\(0.01, 2.56)} & \makecell{0.53\\(0.01, 2.93)} & \makecell{3.77\\(1.52, 8.33)} & \makecell{2.25\\(0.04, 5.34)} & \makecell{0.76\\(0.01, 4.67)} & \makecell{0.79\\(0.01, 4.21)} \\
\makecell{200} & \makecell{7.72\\(0.19, 99.53)} & \makecell{14.48\\(0.12, 201.02)} & \makecell{7.37\\(0.13, 103.08)} & \makecell{7.21\\(0.17, 92.67)} & \makecell{7.41\\(0.05, 95.68)} & \makecell{4.95\\(2.26, 9.65)} & \makecell{10.14\\(1.86, 112.24)} & \makecell{22.26\\(0.11, 261.22)} & \makecell{30.03\\(0.11, 407.12)} \\
\makecell{300} & \makecell{32.44\\(0.56, 249.32)} & \makecell{60.42\\(0.30, 325.82)} & \makecell{38.91\\(0.26, 340.05)} & \makecell{67.42\\(0.25, 398.85)} & \makecell{50.69\\(0.25, 459.07)} & \makecell{56.29\\(3.07, 362.71)} & \makecell{67.10\\(1.61, 574.59)} & \makecell{70.06\\(0.10, 425.06)} & \makecell{64.40\\(0.10, 403.78)} \\
\makecell{400} & \makecell{6.88\\(0.38, 59.87)} & \makecell{10.60\\(0.42, 79.02)} & \makecell{8.49\\(0.18, 70.39)} & \makecell{6.04\\(0.17, 56.54)} & \makecell{7.88\\(0.10, 62.81)} & \makecell{12.80\\(2.92, 52.04)} & \makecell{10.83\\(1.40, 73.03)} & \makecell{38.65\\(0.44, 231.27)} & \makecell{55.98\\(0.44, 367.16)} \\
\makecell{500} & \makecell{32.75\\(0.37, 399.83)} & \makecell{55.10\\(0.50, 437.00)} & \makecell{50.85\\(0.31, 384.77)} & \makecell{45.95\\(0.55, 376.85)} & \makecell{42.20\\(0.23, 430.87)} & \makecell{89.58\\(2.04, 561.99)} & \makecell{31.85\\(1.23, 218.04)} & \makecell{101.84\\(0.83, 336.29)} & \makecell{129.58\\(0.68, 476.19)} \\
\bottomrule
\end{tabular}
\caption{Mean (lowest, highest) runtime for version 2 solvers (SAT)}
\label{tab:mean_low_high_SAT_v2}
\end{table}

\section{CP-SAT Formulation Comparison}
In this section, we will compare using the MIP formulation against the SAT formulation with and without the clauses \cref{eq_SAT:f.3}(CAGP)/\cref{eq_SAT_cf:f.3}(CFCAGP) for the CP-SAT solver. We use the same instances as

\begin{table}[htbp]
\fontsize{4}{4}\selectfont
\centering
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{3} % Increase row spacing
\begin{tabular}{llll}
\toprule
\makecell{vertices} & \makecell{CPSAT MIP} & \makecell{CPSAT SAT\\(version 1)} & \makecell{CPSAT SAT\\(version 2)} \\
% \makecell{vertices} &  &  &  \\
\midrule
\makecell{100} & \makecell{0.61\\(0.13, 1.97)} & \makecell{0.58\\(0.09, 2.36)} & \makecell{0.50\\(0.10, 1.70)} \\
\makecell{200} & \makecell{9.12\\(0.69, 79.63)} & \makecell{8.97\\(0.44, 97.38)} & \makecell{9.87\\(0.40, 107.19)} \\
\makecell{300} & \makecell{36.19\\(3.17, 198.38)} & \makecell{39.69\\(1.40, 249.66)} & \makecell{43.49\\(0.99, 370.79)} \\
\makecell{400} & \makecell{25.68\\(7.67, 115.43)} & \makecell{14.58\\(1.64, 80.00)} & \makecell{11.18\\(1.29, 73.82)} \\
\makecell{500} & \makecell{69.27\\(16.03, 286.05)} & \makecell{67.91\\(4.67, 425.07)} & \makecell{64.40\\(2.78, 443.01)} \\
\bottomrule
\end{tabular}
\caption{Mean (lowest, highest) runtime for solvers (CP-SAT)}
\label{tab:mean_low_high_CPSAT}
\end{table}

\begin{figure}[htbp]
\centering
\includegraphics[scale=0.7]{Thesis/figures/minibenchmark_cactus_plot_runtime_CPSAT_with_holes.png}
\caption{Cactus Plot Comparing the CP-SAT Solver Performance}
\label{fig:cactus_CP-SAT}
\end{figure}

\section{MIP Parameter Set}
Gurobi allows us to set various parameters, which can help improve the runtime of the solver depending on the model. During testing, we used the model.tune() function on a few handpicked instances to determine a good parameter set. Note that setting the parameters drastically improves the runtime of the solver and one might want to further improve them in the future. In the following, we will list the parameters used for the CAGP MIP solver:
\begin{itemize}
  \item Method = 0: primal simplex
  \item NumericFocus = 2: degree to which the code tries to detect and manage numerical issues (0-3)
  \item Presolve = 2: degree of presolve (0-2)
  \item PreSparsify = 0: no presolve sparsify reduction
  \item Cuts = 0: no cuts
  \item Heuristics = 0: no time spent in heuristics
  \item VarBranch = 1: Pseudo Shadow Price Branching
\end{itemize}

\section{MIP vs. SAT vs. CP-SAT}
In this section, we will compare the performance of the MIP, SAT, and CP-SAT solver against each other on various polygon types.

\subsection{Simple Polygons}

\begin{figure}[htbp]
\centering
\includegraphics[scale=0.7]{Thesis/figures/final_benchmark_cactus_plot_runtime_MIP_SAT_CPSAT_without_holes.png}
\caption{Cactus Plot Comparing the MIP, SAT and CP-SAT Solver Performance on simple polygons without holes}
\label{fig:cactus_all_simple_no_holes}
\end{figure}

\begin{table}[htbp]
\fontsize{4}{4}\selectfont
\centering
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{3} % Increase row spacing
\begin{tabular}{llllll}
\toprule
\makecell{vertices} & \makecell{CPSAT MIP} & \makecell{CPSAT SAT\\(version 1)} & \makecell{CPSAT SAT\\(version 2)} & \makecell{MIP} & \makecell{SAT} \\
% \makecell{vertices} &  &  &  &  &  \\
\midrule
\makecell{100} & \makecell{0.06\\(0.03, 0.20)} & \makecell{0.03\\(0.02, 0.13)} & \makecell{0.04\\(0.03, 0.08)} & \makecell{0.04\\(0.02, 0.09)} & \makecell{0.00\\(0.00, 0.00)} \\
\makecell{200} & \makecell{0.26\\(0.07, 0.84)} & \makecell{0.11\\(0.04, 0.24)} & \makecell{0.12\\(0.06, 0.34)} & \makecell{0.12\\(0.06, 0.23)} & \makecell{0.01\\(0.00, 0.02)} \\
\makecell{300} & \makecell{0.53\\(0.16, 1.39)} & \makecell{0.19\\(0.10, 0.46)} & \makecell{0.21\\(0.10, 0.80)} & \makecell{0.25\\(0.11, 0.88)} & \makecell{0.01\\(0.00, 0.10)} \\
\makecell{400} & \makecell{1.03\\(0.26, 2.34)} & \makecell{0.46\\(0.14, 3.02)} & \makecell{0.41\\(0.15, 2.40)} & \makecell{0.41\\(0.19, 0.94)} & \makecell{0.01\\(0.00, 0.06)} \\
\makecell{500} & \makecell{1.67\\(0.39, 3.48)} & \makecell{0.57\\(0.18, 2.40)} & \makecell{0.54\\(0.18, 1.08)} & \makecell{0.59\\(0.25, 1.49)} & \makecell{0.02\\(0.00, 0.05)} \\
\makecell{600} & \makecell{3.64\\(0.62, 8.31)} & \makecell{1.03\\(0.28, 4.86)} & \makecell{0.80\\(0.22, 2.54)} & \makecell{0.94\\(0.39, 1.85)} & \makecell{0.03\\(0.01, 0.12)} \\
\makecell{700} & \makecell{4.87\\(1.96, 9.34)} & \makecell{1.36\\(0.39, 4.18)} & \makecell{0.90\\(0.27, 2.00)} & \makecell{0.98\\(0.36, 2.87)} & \makecell{0.03\\(0.01, 0.16)} \\
\makecell{800} & \makecell{5.36\\(0.72, 18.47)} & \makecell{1.61\\(0.34, 5.55)} & \makecell{1.17\\(0.38, 3.36)} & \makecell{1.19\\(0.54, 2.38)} & \makecell{0.03\\(0.01, 0.05)} \\
\makecell{900} & \makecell{9.22\\(2.74, 17.79)} & \makecell{1.70\\(0.39, 5.61)} & \makecell{1.56\\(0.43, 5.55)} & \makecell{1.52\\(0.59, 2.98)} & \makecell{0.04\\(0.01, 0.11)} \\
\makecell{1000} & \makecell{8.78\\(1.23, 21.35)} & \makecell{1.91\\(0.45, 11.39)} & \makecell{1.47\\(0.46, 5.68)} & \makecell{1.85\\(0.84, 3.38)} & \makecell{0.03\\(0.01, 0.08)} \\
\makecell{1250} & \makecell{20.20\\(8.10, 40.86)} & \makecell{2.80\\(0.87, 12.32)} & \makecell{2.32\\(0.93, 7.30)} & \makecell{2.62\\(1.44, 7.02)} & \makecell{0.04\\(0.01, 0.09)} \\
\makecell{1500} & \makecell{28.20\\(7.18, 59.46)} & \makecell{4.01\\(1.15, 14.92)} & \makecell{3.68\\(1.16, 11.44)} & \makecell{3.13\\(1.69, 5.59)} & \makecell{0.05\\(0.02, 0.37)} \\
\makecell{1750} & \makecell{38.47\\(13.57, 63.67)} & \makecell{4.03\\(1.26, 22.03)} & \makecell{3.72\\(1.37, 13.70)} & \makecell{4.00\\(2.28, 10.73)} & \makecell{0.05\\(0.02, 0.09)} \\
\makecell{2000} & \makecell{46.19\\(11.48, 80.20)} & \makecell{5.24\\(1.90, 31.53)} & \makecell{4.86\\(2.09, 21.83)} & \makecell{4.64\\(2.71, 8.56)} & \makecell{0.05\\(0.02, 0.13)} \\
\makecell{2250} & \makecell{63.21\\(32.91, 113.89)} & \makecell{5.69\\(2.41, 39.52)} & \makecell{5.27\\(2.40, 19.88)} & \makecell{5.83\\(3.55, 13.08)} & \makecell{0.07\\(0.03, 0.13)} \\
\makecell{2500} & \makecell{68.61\\(29.46, 120.69)} & \makecell{6.64\\(3.50, 31.91)} & \makecell{6.43\\(2.97, 12.53)} & \makecell{7.81\\(3.26, 16.56)} & \makecell{0.07\\(0.04, 0.15)} \\
\bottomrule
\end{tabular}
\caption{Mean (lowest, highest) runtime for MIP, SAT, and CP-SAT solvers on random simple polygons without holes}
\label{tab:mean_low_high_all_simple_no_holes}
\end{table}

\subsection{Simple Polygons with Holes}

\begin{figure}[htbp]
\centering
\includegraphics[scale=0.7]{Thesis/figures/final_benchmark_cactus_plot_runtime_MIP_SAT_CPSAT_with_holes.png}
\caption{Cactus Plot Comparing the MIP, SAT, and CP-SAT Solver Performance on simple polygons with holes}
\label{fig:cactus_all_simple_with_holes}
\end{figure}

\begin{table}[htbp]
\fontsize{4}{4}\selectfont
\centering
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{3} % Increase row spacing
\begin{tabular}{lrrrrr}
\toprule
\makecell{vertices} & \makecell{CPSAT MIP} & \makecell{CPSAT SAT (version 1)} & \makecell{CPSAT SAT (version 2)} & \makecell{MIP} & \makecell{SAT} \\
% \makecell{vertices} &  &  &  &  &  \\
\midrule
\makecell{100} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} \\
\makecell{200} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} \\
\makecell{300} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} \\
\makecell{400} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{30} \\
\makecell{500} & \makecell{30} & \makecell{30} & \makecell{30} & \makecell{26} & \makecell{30} \\
\makecell{600} & \makecell{27} & \makecell{28} & \makecell{27} & \makecell{29} & \makecell{28} \\
\makecell{700} & \makecell{29} & \makecell{29} & \makecell{29} & \makecell{27} & \makecell{29} \\
\makecell{800} & \makecell{30} & \makecell{29} & \makecell{30} & \makecell{26} & \makecell{30} \\
\makecell{900} & \makecell{30} & \makecell{29} & \makecell{28} & \makecell{27} & \makecell{30} \\
\makecell{1000} & \makecell{27} & \makecell{26} & \makecell{27} & \makecell{25} & \makecell{26} \\
\bottomrule
\end{tabular}
\caption{Caption}
\label{tab:my_label}
\end{table}

\begin{table}[htbp]
\fontsize{4}{4}\selectfont
\centering
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{3} % Increase row spacing
\begin{tabular}{llrrrrr}
\toprule
 & \makecell{min\_solver} & \makecell{CPSAT MIP} & \makecell{CPSAT SAT (version 1)} & \makecell{CPSAT SAT (version 2)} & \makecell{MIP} & \makecell{SAT} \\
\makecell{vertices} & \makecell{holes} &  &  &  &  &  \\
\midrule
\makecell{100} & \makecell{10} & \makecell{0} & \makecell{0} & \makecell{0} & \makecell{12} & \makecell{18} \\
\cline{1-7}
\makecell{200} & \makecell{20} & \makecell{0} & \makecell{0} & \makecell{0} & \makecell{9} & \makecell{21} \\
\cline{1-7}
\makecell{300} & \makecell{30} & \makecell{0} & \makecell{0} & \makecell{0} & \makecell{15} & \makecell{15} \\
\cline{1-7}
\makecell{400} & \makecell{40} & \makecell{0} & \makecell{0} & \makecell{0} & \makecell{4} & \makecell{26} \\
\cline{1-7}
\makecell{500} & \makecell{50} & \makecell{2} & \makecell{1} & \makecell{0} & \makecell{8} & \makecell{19} \\
\cline{1-7}
\makecell{600} & \makecell{60} & \makecell{0} & \makecell{0} & \makecell{1} & \makecell{12} & \makecell{16} \\
\cline{1-7}
\makecell{700} & \makecell{70} & \makecell{0} & \makecell{1} & \makecell{1} & \makecell{7} & \makecell{21} \\
\cline{1-7}
\makecell{800} & \makecell{80} & \makecell{0} & \makecell{0} & \makecell{1} & \makecell{3} & \makecell{26} \\
\cline{1-7}
\makecell{900} & \makecell{90} & \makecell{0} & \makecell{1} & \makecell{0} & \makecell{2} & \makecell{27} \\
\cline{1-7}
\makecell{1000} & \makecell{100} & \makecell{1} & \makecell{0} & \makecell{1} & \makecell{4} & \makecell{22} \\
\cline{1-7}
\bottomrule
\end{tabular}
\caption{Caption}
\label{tab:my_label}
\end{table}

\begin{table}[htbp]
\fontsize{4}{4}\selectfont
\centering
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{3} % Increase row spacing
\begin{tabular}{llllll}
\toprule
\makecell{label} & \makecell{CPSAT MIP} & \makecell{CPSAT SAT \\ (version 1)} & \makecell{CPSAT SAT \\ (version 2)} & \makecell{MIP} & \makecell{SAT} \\
\makecell{vertices} &  &  &  &  &  \\
\midrule
\makecell{100} & \makecell{0.50 \\ (0.12, 1.28)} & \makecell{0.61 \\ (0.06, 2.03)} & \makecell{0.56 \\ (0.06, 2.71)} & \makecell{0.29 \\ (0.10, 0.78)} & \makecell{0.29 \\ (0.01, 1.29)} \\
\makecell{200} & \makecell{8.86 \\ (0.40, 85.41)} & \makecell{9.96 \\ (0.38, 118.32)} & \makecell{9.94 \\ (0.46, 111.35)} & \makecell{1.60 \\ (0.24, 6.53)} & \makecell{7.38 \\ (0.08, 112.68)} \\
\makecell{300} & \makecell{38.26 \\ (2.91, 241.32)} & \makecell{36.92 \\ (1.18, 248.56)} & \makecell{41.70 \\ (1.36, 341.01)} & \makecell{8.46 \\ (0.97, 39.76)} & \makecell{31.67 \\ (0.16, 221.74)} \\
\makecell{400} & \makecell{22.44 \\ (4.63, 77.96)} & \makecell{11.14 \\ (1.80, 61.91)} & \makecell{12.27 \\ (1.77, 61.64)} & \makecell{7.58 \\ (1.36, 25.27)} & \makecell{5.12 \\ (0.26, 50.69)} \\
\makecell{500} & \makecell{66.71 \\ (15.26, 272.99)} & \makecell{53.97 \\ (3.56, 418.28)} & \makecell{60.57 \\ (4.13, 441.61)} & \makecell{21.83 \\ (3.60, 192.12)} & \makecell{41.87 \\ (0.23, 484.14)} \\
\makecell{600} & \makecell{96.87 \\ (23.21, 380.71)} & \makecell{88.33 \\ (5.50, 495.50)} & \makecell{80.77 \\ (6.55, 343.77)} & \makecell{33.47 \\ (3.72, 117.94)} & \makecell{33.13 \\ (0.65, 166.00)} \\
\makecell{700} & \makecell{126.96 \\ (38.10, 347.75)} & \makecell{82.98 \\ (16.03, 349.98)} & \makecell{78.52 \\ (11.23, 520.49)} & \makecell{67.92 \\ (12.32, 419.44)} & \makecell{44.18 \\ (1.42, 242.68)} \\
\makecell{800} & \makecell{160.95 \\ (49.61, 555.81)} & \makecell{87.54 \\ (11.58, 273.35)} & \makecell{92.02 \\ (6.39, 378.87)} & \makecell{86.64 \\ (19.47, 551.34)} & \makecell{48.28 \\ (1.65, 541.23)} \\
\makecell{900} & \makecell{227.93 \\ (65.70, 584.75)} & \makecell{127.28 \\ (15.94, 557.20)} & \makecell{107.13 \\ (25.91, 461.89)} & \makecell{93.58 \\ (18.99, 501.60)} & \makecell{37.16 \\ (1.15, 232.08)} \\
\makecell{1000} & \makecell{201.89 \\ (62.86, 518.47)} & \makecell{127.80 \\ (31.10, 453.32)} & \makecell{119.97 \\ (28.17, 553.33)} & \makecell{107.49 \\ (11.93, 417.70)} & \makecell{31.30 \\ (4.19, 181.87)} \\
\bottomrule
\end{tabular}
\caption{Caption}
\label{tab:my_label}
\end{table}